...
def classify_words(word_dict):
    """词语分类,找出情感词、否定词、程度副词"""
    # 读取情感字典文件
    sen_file = open('./data/BosonNLP_sentiment_score/BosonNLP_sentiment_score.txt', 'r+', encoding='utf-8')
    # 获取字典文件内容
    sen_list = [line.strip() for line in sen_file.readlines()]
    # 创建情感字典
    ...
    # 读取否定词文件
    not_word_file = open('./data/副词/否定词.txt', 'r+')
    # 由于否定词只有词，没有分值，使用list即可
    not_word_list = not_word_file.readlines()

    # 读取程度副词文件
    with open('./data/副词/程度副词.txt', 'r') as f:
        degreeList = [line.strip() for line in f.readlines()]
...
# 计算得分
def setiment_score(tags):
    # 1.对文档分词
    score = []
    n = len(tags)
    j = 0
    print('Now calculating emotion scores...')
    for i in tags:
        seg_list = i
        # 2.将分词结果列表转为dic，然后找出情感词、否定词、程度副词
        sen_word, not_word, degree_word = classify_words(list_to_dict(seg_list))
        # 3.计算得分
        score.append(score_sentiment(sen_word, not_word, degree_word, seg_list))
        if j%100==0:
            print('%d/%d completed!'%(j,n))
        j += 1
    print('All completed!')
    return score
...
# 测试
def main():
    file_list = ['pku_foreign_tags', 'pku_ted_tags']
    for file in file_list:
        print('+++++++++Now dealing with %s++++++++++++++'%file)
        df = cal_weight('./data/'+file+'.csv').dropna()
        print('Completed!')
        tag = df['Content']
        score = setiment_score(tag)
        df['score'] = score
        df['weighted_score'] = df['Weight']*df['score']
        name = './score/%s_score.csv'%file
